...
| **Retrieval** | Grouped and aggregated at read time — `student_model.py`'s `_quiz_signals()` for weak/strong areas, `streaks.py::compute_streak` for the activity heatmap, and (once built) `plan-backend.md §11`'s confusion-pair aggregation over the same `quiz_results` rows |
